...
import json
import numpy as np 
# database removed — no MySQL needed
...
def submit_business_profile():
    ...
    try:
        ...
        # No MySQL needed - generate ID from timestamp
        import time
        business_id = int(time.time())
...
def get_history(business_id):
    try:
        # History not stored (no DB) - return empty
        return jsonify({'business': None, 'analyses': []}), 200
    except Exception as e:
        return jsonify({'error':str(e)}),500
